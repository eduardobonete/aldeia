...
export default {
			...
			this.carts.forEach((cart) => {
				...
				if (this.verifyAndSumValue(cartObject)) {
					this.sellers.push({
						...
						seller_id: cartObject.seller_id,
	...
	methods: {
		...
		setParcelas(i) {
			var installments = {}
			i.forEach((row) => {
				installments[row.seller_id.toString()] = parseInt(row.installments);
			})
			this.$store.dispatch('payment/setParcelas', installments).then(() => {
...
<template>
	...
	<div class="mt-100">
		...
		<div v-if="!isLoading">
			<div v-if="sellers.length > 1"
				style="display: flex; text-align: center; background-color: rgb(243,107,45); color: #FFFFFF;font-size: 12px;padding-top: 1rem; padding-bottom: 1rem  ;">
				<p>Você escolheu produtos com diferentes opções de parcelamento. Selecione em quantas vezes
					você quer parcelar
					cada pagamento.</p>
			</div>
